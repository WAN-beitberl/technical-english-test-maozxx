...
export default function ConnectFour() {
	...
	function createBoard() {
		return new Array(boardSettings.rows * boardSettings.columns).fill(boardSettings.colors.empty);
	}
	...
	async function animateDrop(row, column, color, currentRow) {
		...
		return new Promise((resolve) => {
			...
			setTimeout(() => resolve(animateDrop(row, column, color, ++currentRow)), boardSettings.dropAnimationRate);
		});
	}
	...
	useEffect(() => {
		...
		function flashWinningCells(on) {
			...
			setFlashTimer(setTimeout(() => flashWinningCells(!on), boardSettings.flashAnimationRate));
		}
	...
	useEffect(() => {
		...
		function isWin() {
			return isForwardsDiagonalWin() || isBackwardsDiagonalWin() || isHorizontalWin() || isVerticalWin() || null;
		}
				...
				for (let column = 0; column <= columns - 4; column++) {
					...
					for (let i = 1; i < 4; i++) {
						if (board[getIndex(row + i, column + i)] === board[start]) {
							counter++;
							if (counter === 4) return createWinState(start, winTypes.backwardsDiagonal);
						}
					}
				}
			}
			...
			for (let row = 0; row <= rows - 4; row++) {
				for (let column = 3; column <= columns; column++) {
					...
					for (let i = 1; i < 4; i++) {
						if (board[getIndex(row + i, column - i)] === board[start]) {
							counter++;
							if (counter === 4) return createWinState(start, winTypes.forwardsDiagonal);
						}
					}
				}
			}
	...
	return (
		<>
			...
			{!win && <h2 style={{ color: currentPlayer }}>{currentPlayer === boardSettings.colors.p1 ? 'Player 1' : 'Player 2'}</h2>}
			{win && (
				<>
					<h1 style={{ color: win.winner }}> {win.winner === boardSettings.colors.p1 ? 'Player 1' : 'Player 2'} WON!</h1>
